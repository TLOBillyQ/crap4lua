...
function common.make_temp_path(prefix, suffix)
  ...
  local base_dir = common.join_path(common.system_tmp_dir(), "crap4lua")
...
function common.read_file(path)
  ...
  if not common.is_windows() then
    ...
    if content == nil then
      return nil, common.bilingual(
        "Unable to open file: " .. tostring(path),
        "Unable to open file: " .. tostring(path)
      )
    end
    return content
  ...
  if content == nil then
    return nil, common.bilingual(
      "Unable to read temporary output: " .. tostring(path),
      "Unable to read temporary output: " .. tostring(path)
    )
  end
  return content
...
function common.write_file(path, content)
  ...
  if not common.is_windows() then
    ...
    if write_ok == nil then
      return nil, common.bilingual(
        "Unable to write file: " .. tostring(path),
        "Unable to write file: " .. tostring(path)
      )
    end
    return true
  ...
  if wrote_temp == nil then
    return nil, common.bilingual(
      "Unable to write temporary file: " .. tostring(temp_path),
      "Unable to write temporary file: " .. tostring(temp_path)
    )
  end
...
function common.run_command(command, options)
  ...
  if type(args) ~= "table" or #args == 0 then
    return {
      ok = false,
      code = 1,
      output = common.bilingual("Invalid command arguments", "Invalid command arguments"),
    }
  end
